  ...
  function renderAthDashboard(){
    ...
    document.getElementById('panel-tableau-de-bord').innerHTML = `
      <div class="ath-dash" id="ath-dash">
        <div id="ath-stats-row" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px"></div>
        <!-- Planning semaine -->
  ...
  function athRenderStats(){
    const el = document.getElementById('ath-stats-row');
    if(!el || !athData) return;

    const weekDays = getWeekDays(0);
    const weekKeys = new Set(weekDays.map(d => dateToKey(d)));
    const d30ago = new Date(); d30ago.setDate(d30ago.getDate()-29); d30ago.setHours(0,0,0,0);
    const d30Key = dateToKey(d30ago);

    const comps = athData.completions || {};
    const weekCount = Object.keys(comps).filter(k => weekKeys.has(k.split('|')[1])).length;
    const last30Count = Object.keys(comps).filter(k => k.split('|')[1] >= d30Key).length;

    const pill = (icon, value, label, accent) => `
      <div style="background:${accent}0d;border:1px solid ${accent}33;border-radius:10px;padding:8px 14px;display:flex;align-items:center;gap:8px">
        <span style="color:${accent};font-size:13px">${icon}</span>
        <span style="font-size:12px;font-weight:700;color:var(--white)">${value}</span>
        <span style="font-size:11px;color:rgba(240,236,228,.35)">${label}</span>
      </div>`;

    // Anneau de progression semaine (objectif 6 séances)
    const MAX = 6;
    const pct = Math.min(weekCount / MAX, 1);
    const R = 15, CIRC = +(2 * Math.PI * R).toFixed(2);
    const offset = +(CIRC * (1 - pct)).toFixed(2);
    const done = weekCount >= MAX;
    const ringColor = weekCount === 0 ? 'rgba(240,236,228,.15)'
      : weekCount <= 2 ? '#2ec4b6'
      : weekCount <= 4 ? '#90be6d'
      : weekCount <= 5 ? '#f4a261'
      : '#34d399';
    ...
    const ring = `
      <div style="display:flex;align-items:center;gap:10px;background:${done?'rgba(52,211,153,.07)':'rgba(255,255,255,.03)'};border:1px solid ${done?'rgba(52,211,153,.28)':'rgba(255,255,255,.08)'};border-radius:10px;padding:6px 14px 6px 8px">
        <svg viewBox="0 0 38 38" width="44" height="44" style="flex-shrink:0;overflow:visible">
          <circle cx="19" cy="19" r="${R}" fill="none" stroke="rgba(255,255,255,.07)" stroke-width="4"/>
          ...
          <text x="19" y="24" text-anchor="middle"
            font-family="'Bebas Neue',sans-serif" font-size="14" fill="${ringColor}">${weekCount}</text>
        </svg>
        <div>
          <div style="font-size:12px;font-weight:700;color:${ringColor};line-height:1.2">${done ? '🏆 Complète !' : weekCount + ' / ' + MAX}</div>
          <div style="font-size:9px;color:rgba(240,236,228,.3);letter-spacing:.08em;text-transform:uppercase;margin-top:2px">séances / semaine</div>
        </div>
      </div>`;

    el.innerHTML =
      pill('✓', weekCount + ' séance' + (weekCount!==1?'s':''), 'cette semaine', '#2ec4b6') +
      pill('📅', last30Count + ' séance' + (last30Count!==1?'s':''), 'sur 30 jours', '#f4a261') +
      ring;
  }
  ...
  function athRenderSeances(){
    ...
    if(!seancesInWeek.length){
      el.innerHTML = `<div class="adp-empty"><div class="adp-empty-icon">🏋️</div><div class="adp-empty-title">Aucune séance cette semaine</div>Votre coach n'a pas encore planifié de séances pour cette semaine.</div><div id="ath-seance-chart-wrap" style="margin-top:20px"></div>`;
      renderAthSeanceChart();
      return;
    }
    ...
    const chartHtml = `<div id="ath-seance-chart-wrap" style="margin:20px 0"></div>`;
    ...
    el.innerHTML = todoHtml + chartHtml + doneHtml;
    renderAthSeanceChart();
    athRenderStats();
  }
  ...
  function copyAthleteCode(code){
    ...
    if(navigator.clipboard && navigator.clipboard.writeText){
      navigator.clipboard.writeText(code).then(confirm).catch(()=>_fallbackCopy(code,confirm));
    } else {
      _fallbackCopy(code, confirm);
    }
  }
  ...
  async function refreshCompletionBadge(){
    ...
    try {
      ...
      const _d30 = new Date(); _d30.setDate(_d30.getDate()-29); _d30.setHours(0,0,0,0);
      const last30Count = (rows||[]).filter(r => r.schedule_date >= dateToKey(_d30)).length;

      // Badge header — toujours recalculé depuis rows frais
      const compBadge = document.getElementById('adp-comp-badge');
      if(compBadge){
        const weekBadge = `<span class="completion-badge${count===0?' none':''}">${count>0?'✓ ':''} ${count} séance${count!==1?'s':''} cette semaine</span>`;
        const monthBadge = `<span class="completion-badge" style="background:rgba(244,162,97,.08);border-color:rgba(244,162,97,.25);color:rgba(244,162,97,.8)">📅 ${last30Count} sur 30j</span>`;
        compBadge.innerHTML = `<div style="display:flex;gap:8px;flex-wrap:wrap">${weekBadge}${monthBadge}</div>`;
      }
    ...
    el.innerHTML = Object.entries(byDate)
      ...
      .map(([date, comps])=>{
        ...
        const items = comps.map(r=>{
          ...
          const rpeVal = r.rpe_ressenti;
          const rpeHtml = (rpeVal !== null && rpeVal !== undefined)
            ? `<span style="font-family:'Bebas Neue',sans-serif;font-size:15px;color:${RPE_COLORS[rpeVal]};background:${RPE_COLORS[rpeVal]}22;border:1px solid ${RPE_COLORS[rpeVal]}55;border-radius:5px;padding:0 6px;flex-shrink:0" title="RPE ressenti">${rpeVal}</span>`
            : `<span style="font-size:10px;color:rgba(240,236,228,.2);flex-shrink:0">RPE —</span>`;
          return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.04)">
            <span style="color:#2ec4b6;font-size:14px;flex-shrink:0">✓</span>
            <span style="flex:1;font-size:12px;color:rgba(240,236,228,.7);min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" id="compseance-${r.seance_id}">${r.seance_id}</span>
            <span style="font-size:11px;color:rgba(240,236,228,.25);flex-shrink:0">${t}</span>
            ${rpeHtml}
          </div>`;
        }).join('');
  ...
  async function openAthleteDetail(athleteId){
    ...
    const _d30init = new Date(); _d30init.setDate(_d30init.getDate()-29); _d30init.setHours(0,0,0,0);
    const last30init = (a.completions||[]).filter(r => r.schedule_date >= dateToKey(_d30init)).length;
    const compBadge = document.getElementById('adp-comp-badge') || (() => {
    ...
    const _wc = weekComps.length;
    compBadge.innerHTML = `<div style="display:flex;gap:8px;flex-wrap:wrap">
      <span class="completion-badge${_wc===0?' none':''}">${_wc>0?'✓ ':''} ${_wc} séance${_wc!==1?'s':''} cette semaine</span>
      <span class="completion-badge" style="background:rgba(244,162,97,.08);border-color:rgba(244,162,97,.25);color:rgba(244,162,97,.8)">📅 ${last30init} sur 30j</span>
    </div>`;
  ...
  function renderCoachAthleteProfile(a, editMode){
    ...
    const adpChartHtml = `<div id="adp-profil-chart-section" style="margin-top:20px"></div>`;
    document.getElementById('adp-profile-grid').innerHTML = codeHtml + gridHtml + editHtml + usernameHtml + adpChartHtml;
  ...
  function closeAthleteDetail(){
    stopCompPolling();
    if(_adpAthleteChart){ _adpAthleteChart.destroy(); _adpAthleteChart = null; }
    if(_chargeChart){ _chargeChart.destroy(); _chargeChart = null; }
  ...
  function switchAthleteTab(name, btn){
    ...
    if(name==='profil' && activeAthleteId) renderAdpAthleteChart(activeAthleteId);
    if(name==='charge' && activeAthleteId) renderChargePanel(activeAthleteId);
  }
  ...
  async function renderInlineWeek(athlete){
    ...
    document.getElementById('iw-grid').innerHTML = days.map((d,i) => {
      ...
      const pillsHtml = slots.map(slot=>{
        ...
        const canDrag = slot.scheduleId && !String(slot.scheduleId).startsWith('temp');
        return `<div class="inline-pill${isComp?' completed':''}"
          data-sid="${slot.sid}" data-schedid="${slot.scheduleId}" data-date="${key}"
          style="border-left:3px solid ${slot.color||'#2ec4b6'};background:${isComp?'rgba(46,196,182,.12)':slot.color+'22'}"
          title="${escHtml(slot.title)}"
          ${canDrag?`onpointerdown="iwPillDown(event,'${key}','${slot.sid}','${slot.scheduleId}')"` : ''}
  ...
  function openDayPageFromInline(key, label, e){
    e.stopPropagation();
    if(_iwJustDragged){ _iwJustDragged = false; return; }
  ...
  async function inlineDayAdd(e, dateKey){
    e.stopPropagation();
    if(!weekSeanceId){ openSeancePicker(e, dateKey); return; }
  ...
  /* ══ INLINE WEEK DRAG & DROP ══ */
  let _iwDrag       = null;   // { dateKey, sid, scheduleId, slot }
  let _iwPending    = null;   // même structure, en attente du long press
  let _iwGhost      = null;   // élément fantôme
  let _iwSourceEl   = null;   // pill source
  let _iwDropInfo   = null;   // { dateKey, position }
  let _iwTimer      = null;
  let _iwStart      = null;   // { x, y }
  let _iwJustDragged = false;

  function iwPillDown(e, dateKey, sid, scheduleId){
    e.stopPropagation();
    e.preventDefault();
    _iwStart = { x: e.clientX, y: e.clientY };

    const data = window._lastCoachData;
    const athlete = data?.athletes.find(a => a.id === activeAthleteId);
    if(!athlete) return;
    const slot = (athlete.schedule[dateKey]||[]).find(s => s.sid === sid && s.scheduleId === scheduleId);
    if(!slot) return;

    _iwPending = { dateKey, sid, scheduleId, slot };

    _iwTimer = setTimeout(() => {
      _iwTimer = null;
      if(!_iwPending) return;
      iwActivate(e.clientX, e.clientY);
    }, 480);

    document.addEventListener('pointermove', iwPreMove);
    document.addEventListener('pointerup',   iwPreCancel);
    document.addEventListener('pointercancel', iwPreCancel);
  }

  function iwPreMove(e){
    if(!_iwStart) return;
    const dx = e.clientX - _iwStart.x, dy = e.clientY - _iwStart.y;
    if(Math.sqrt(dx*dx+dy*dy) > 10) iwPreCancel();
  }

  function iwPreCancel(){
    if(_iwTimer){ clearTimeout(_iwTimer); _iwTimer = null; }
    _iwPending = null; _iwStart = null;
    document.removeEventListener('pointermove', iwPreMove);
    document.removeEventListener('pointerup',   iwPreCancel);
    document.removeEventListener('pointercancel', iwPreCancel);
  }

  function iwActivate(x, y){
    document.removeEventListener('pointermove', iwPreMove);
    document.removeEventListener('pointerup',   iwPreCancel);
    document.removeEventListener('pointercancel', iwPreCancel);

    _iwDrag = _iwPending; _iwPending = null;
    document.body.style.userSelect = 'none';
    document.body.style.webkitUserSelect = 'none';
    if(navigator.vibrate) navigator.vibrate(60);

    _iwSourceEl = document.querySelector(`.inline-pill[data-sid="${_iwDrag.sid}"][data-schedid="${_iwDrag.scheduleId}"]`);
    if(_iwSourceEl) _iwSourceEl.classList.add('iw-dragging');

    const { slot } = _iwDrag;
    _iwGhost = document.createElement('div');
    _iwGhost.id = 'iw-ghost';
    _iwGhost.className = 'inline-pill';
    _iwGhost.style.cssText = `position:fixed;pointer-events:none;z-index:9999;min-width:70px;max-width:130px;border-left:3px solid ${slot.color||'#2ec4b6'};background:${(slot.color||'#2ec4b6')}33;box-shadow:0 8px 32px rgba(0,0,0,.6);transform:scale(1.08) rotate(2deg);opacity:.95;`;
    _iwGhost.innerHTML = `<span style="font-size:10px;font-weight:600;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block">${escHtml(slot.title)}</span>`;
    document.body.appendChild(_iwGhost);
    iwMoveGhost(x, y);

    document.addEventListener('pointermove', iwOnMove, { passive: false });
    document.addEventListener('pointerup',   iwOnEnd);
    document.addEventListener('pointercancel', iwOnEnd);
  }

  function iwMoveGhost(x, y){
    if(!_iwGhost) return;
    const w = _iwGhost.offsetWidth || 90;
    _iwGhost.style.left = (x - w/2) + 'px';
    _iwGhost.style.top  = (y - 30) + 'px';
  }

  function iwOnMove(e){
    if(!_iwDrag) return;
    e.preventDefault();
    iwMoveGhost(e.clientX, e.clientY);
    iwUpdateTarget(e.clientX, e.clientY);
  }

  function iwUpdateTarget(x, y){
    document.querySelectorAll('.iw-drop-line').forEach(el => el.remove());
    document.querySelectorAll('.inline-day.iw-drop-over').forEach(el => el.classList.remove('iw-drop-over'));

    const el = document.elementFromPoint(x, y);
    const dayEl = el?.closest('.inline-day');
    if(!dayEl){ _iwDropInfo = null; return; }
    const dateKey = dayEl.dataset.date;
    if(!dateKey){ _iwDropInfo = null; return; }

    const slotsEl = dayEl.querySelector('.inline-day-slots');
    if(!slotsEl){ _iwDropInfo = null; return; }

    const pills = [...slotsEl.querySelectorAll('.inline-pill')].filter(p => !p.classList.contains('iw-dragging'));

    if(pills.length === 0){
      dayEl.classList.add('iw-drop-over');
      _iwDropInfo = { dateKey, position: 0 };
    } else {
      let insertPos = pills.length, insertBefore = null;
      for(let i = 0; i < pills.length; i++){
        const rect = pills[i].getBoundingClientRect();
        if(y < rect.top + rect.height / 2){ insertPos = i; insertBefore = pills[i]; break; }
      }
      const line = document.createElement('div');
      line.className = 'iw-drop-line';
      insertBefore ? slotsEl.insertBefore(line, insertBefore) : slotsEl.appendChild(line);
      _iwDropInfo = { dateKey, position: insertPos };
    }
  }

  function iwOnEnd(){
    document.removeEventListener('pointermove', iwOnMove);
    document.removeEventListener('pointerup',   iwOnEnd);
    document.removeEventListener('pointercancel', iwOnEnd);

    const drag = _iwDrag, drop = _iwDropInfo;
    iwCleanup();
    if(!drag || !drop) return;

    _iwJustDragged = true;
    setTimeout(() => { _iwJustDragged = false; }, 300);

    if(drag.dateKey === drop.dateKey){
      iwReorder(drag, drop);
    } else {
      iwMoveDay(drag, drop);
    }
  ...
  function iwCleanup(){
    ...
    if(_iwGhost){ _iwGhost.remove(); _iwGhost = null; }
    if(_iwSourceEl){ _iwSourceEl.classList.remove('iw-dragging'); _iwSourceEl = null; }
    document.querySelectorAll('.iw-drop-line').forEach(el => el.remove());
    document.querySelectorAll('.inline-day.iw-drop-over').forEach(el => el.classList.remove('iw-drop-over'));
    _iwDrag = null; _iwDropInfo = null; _iwStart = null;
  }

  async function iwMoveDay(drag, drop){
    try {
      await sbFetch('schedule?id=eq.'+drag.scheduleId, 'PATCH', { date: drop.dateKey, position: drop.position });
      const data = window._lastCoachData;
      const athlete = data?.athletes.find(a => a.id === activeAthleteId);
      if(athlete){
        const remaining = (athlete.schedule[drag.dateKey]||[]).filter(s => s.scheduleId !== drag.scheduleId);
        await Promise.all(remaining.map((s,i) =>
          s.scheduleId && !String(s.scheduleId).startsWith('temp')
            ? sbFetch('schedule?id=eq.'+s.scheduleId, 'PATCH', { position: i })
            : Promise.resolve()
        ));
      }
    } catch(err){ console.error('iwMoveDay:', err); }
    const { athlete } = await getAthleteData();
    if(athlete) renderInlineWeek(athlete);
  }

  async function iwReorder(drag, drop){
    const data = window._lastCoachData;
    const athlete = data?.athletes.find(a => a.id === activeAthleteId);
    if(!athlete) return;
    const slots = [...(athlete.schedule[drag.dateKey]||[])];
    const srcIdx = slots.findIndex(s => s.scheduleId === drag.scheduleId);
    if(srcIdx === -1) return;
    const [moved] = slots.splice(srcIdx, 1);
    let dst = drop.position; if(srcIdx < dst) dst--;
    slots.splice(Math.max(0, dst), 0, moved);
    try {
    ...
    } catch(err){ console.error('iwReorder:', err); }
    const { athlete: fresh } = await getAthleteData();
    if(fresh) renderInlineWeek(fresh);
  }
  ...
  /* ══ GRAPHIQUE ACTIVITÉ — DASHBOARD ATHLÈTE ══ */
  let _athSeanceChart = null;

  function renderAthSeanceChart(){
    const el = document.getElementById('ath-seance-chart-wrap');
    if(!el || !athData) return;
    if(_athSeanceChart){ _athSeanceChart.destroy(); _athSeanceChart = null; }
    ...
    for(let i=4; i>=0; i--){
      ...
      const monKey = dateToKey(mon), sunKey = dateToKey(sun);
      const count = Object.keys(athData.completions || {}).filter(k => {
        const dk = k.split('|')[1];
        return dk >= monKey && dk <= sunKey;
      }).length;
      weeks.push({ label: mon.getDate()+'/'+(mon.getMonth()+1), count });
    }

    el.innerHTML = `
      <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:16px 16px 12px">
        <div style="font-size:10px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:rgba(46,196,182,.6);margin-bottom:12px">Activité — 30 derniers jours</div>
        <div style="position:relative;height:110px">
          <canvas id="ath-seance-chart-canvas"></canvas>
        </div>
      </div>`;

    const canvas = document.getElementById('ath-seance-chart-canvas');
    if(!canvas) return;
    _athSeanceChart = new Chart(canvas.getContext('2d'), {
      type: 'line',
      data: {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor:'#1a1a1a', borderColor:'rgba(255,255,255,.1)', borderWidth:1,
            titleColor:'rgba(240,236,228,.5)', bodyColor:'#2ec4b6',
            callbacks: { label: ctx => ctx.raw + ' séance'+(ctx.raw!==1?'s':'') }
          }
        },
        scales: {
          x: {
            ticks: { color:'rgba(240,236,228,.3)', font:{family:'Outfit',size:10} },
            grid: { display:false }
          },
          y: {
            beginAtZero: true,
            ticks: { color:'rgba(240,236,228,.3)', font:{family:'Outfit',size:10}, stepSize:1, precision:0 },
  ...
  /* ══ ONGLET CHARGE (vue coach) ══ */
  let _chargeChart = null;
  let _chargeCompletions = [];
  let _chargePeriod = '7';

  async function renderChargePanel(athleteId){
    const el = document.getElementById('charge-content');
    if(!el) return;
    if(_chargeChart){ _chargeChart.destroy(); _chargeChart = null; }
    el.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:120px;color:rgba(240,236,228,.3);font-size:13px">⏳ Chargement…</div>`;

    const d21ago = new Date(); d21ago.setDate(d21ago.getDate()-20); d21ago.setHours(0,0,0,0);
    try {
      _chargeCompletions = await sbFetch(
        'seance_completions?athlete_id=eq.'+athleteId+
        '&schedule_date=gte.'+dateToKey(d21ago)+
        '&rpe_ressenti=not.is.null'+
        '&select=schedule_date,rpe_ressenti,duration_minutes'+
        '&order=schedule_date.asc'
      ) || [];
    } catch(e){
      // Fallback : duration_minutes n'existe pas encore dans la table
      try {
        _chargeCompletions = await sbFetch(
          'seance_completions?athlete_id=eq.'+athleteId+
          '&schedule_date=gte.'+dateToKey(d21ago)+
          '&rpe_ressenti=not.is.null'+
          '&select=schedule_date,rpe_ressenti'+
          '&order=schedule_date.asc'
        ) || [];
      } catch(e2){
        el.innerHTML = `<div style="padding:20px;color:#e63946;font-size:13px">Erreur de chargement : ${e2.message}</div>`;
        return;
      }
    }
    renderChargeView();
  ...
  function setChargePeriod(p){ _chargePeriod = p; renderChargeView(); }

  function renderChargeView(){
    const el = document.getElementById('charge-content');
    if(!el) return;
    if(_chargeChart){ _chargeChart.destroy(); _chargeChart = null; }

    const btn = (id, label) => {
      const active = _chargePeriod === id;
      return `<button onclick="setChargePeriod('${id}')" style="padding:7px 16px;border-radius:20px;font-size:12px;font-weight:600;font-family:'Outfit',sans-serif;cursor:pointer;border:1px solid;transition:all .2s;${active?'background:rgba(244,162,97,.15);border-color:#f4a261;color:#f4a261':'background:rgba(255,255,255,.04);border-color:rgba(255,255,255,.1);color:rgba(240,236,228,.4)'}">${label}</button>`;
    };

    el.innerHTML = `
      <div style="display:flex;gap:8px;margin-bottom:24px;flex-wrap:wrap">
        ${btn('7','7 derniers jours')}
        ${btn('14','14 derniers jours')}
        ${btn('21','21 derniers jours')}
        ${btn('acwr','3j / 21j')}
      </div>
      <div id="charge-chart-area"></div>`;

    if(_chargePeriod === '7' || _chargePeriod === '14' || _chargePeriod === '21') renderChargeDaysChart(parseInt(_chargePeriod));
    else renderChargeAcwr();
  }

  function renderChargeDaysChart(n){
    const area = document.getElementById('charge-chart-area');
    if(!area) return;

    const cutoff = new Date(); cutoff.setDate(cutoff.getDate()-(n-1)); cutoff.setHours(0,0,0,0);
    const cutKey = dateToKey(cutoff);

    // sRPE = RPE × durée (si dispo), sinon RPE seul
    const sRPE = c => (c.duration_minutes && c.duration_minutes > 0) ? c.rpe_ressenti * c.duration_minutes : c.rpe_ressenti;
    const hasDuration = _chargeCompletions.some(c => c.duration_minutes && c.duration_minutes > 0);

    const days = [];
    for(let i=n-1; i>=0; i--){
      const d = new Date(); d.setDate(d.getDate()-i); d.setHours(0,0,0,0);
      const key = dateToKey(d);
      const comps = _chargeCompletions.filter(c=>c.schedule_date===key);
      const totalLoad = comps.reduce((s,c)=>s+sRPE(c),0);
      days.push({ label: d.getDate()+'/'+(d.getMonth()+1), load: comps.length ? totalLoad : null, count: comps.length });
    }

    const periodComps = _chargeCompletions.filter(c=>c.schedule_date>=cutKey);
    const total = periodComps.length;
    const totalLoad = periodComps.reduce((s,c)=>s+sRPE(c),0);
    const avgLoad = total ? totalLoad/total : null;
    const maxLoad = total ? Math.max(...periodComps.map(c=>sRPE(c))) : null;
    const maxY = Math.max(hasDuration?200:10, ...(days.map(d=>d.load||0))) * 1.15;
    ...
    const statCard = (label, val, color, sub='') => `
      <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:14px;text-align:center">
        <div style="font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(240,236,228,.3);margin-bottom:6px">${label}</div>
        <div style="font-family:'Bebas Neue',sans-serif;font-size:26px;color:${color};line-height:1">${val}</div>
        ${sub?`<div style="font-size:9px;color:rgba(240,236,228,.25);margin-top:3px">${sub}</div>`:''}
      </div>`;

    const loadLabel = hasDuration ? 'Charge (sRPE)' : 'RPE moy.';
    const loadUnit  = hasDuration ? 'RPE × min' : '';
    area.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px">
        ${statCard('Séances', total, 'var(--white)')}
        ${statCard(loadLabel, avgLoad!==null?avgLoad.toFixed(hasDuration?0:1):'—', avgLoad!==null?RPE_COLORS[Math.min(10,Math.round(hasDuration?avgLoad/30:avgLoad))]:'rgba(240,236,228,.25)', loadUnit)}
        ${statCard('Charge max', maxLoad!==null?Math.round(maxLoad):'—', maxLoad!==null?RPE_COLORS[Math.min(10,Math.round(hasDuration?maxLoad/30:maxLoad))]:'rgba(240,236,228,.25)', loadUnit)}
      </div>
      ${hasDuration?'':`<div style="font-size:11px;color:rgba(244,162,97,.6);background:rgba(244,162,97,.08);border:1px solid rgba(244,162,97,.2);border-radius:8px;padding:8px 12px;margin-bottom:14px">💡 Durée non renseignée — affichage en RPE. La charge sRPE (RPE × durée) s'activera dès que l'athlète entre sa durée.</div>`}
      <div style="position:relative;height:200px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:16px 16px 12px">
        <canvas id="charge-canvas"></canvas>
      </div>`;

    const canvas = document.getElementById('charge-canvas');
    if(!canvas) return;
    _chargeChart = new Chart(canvas.getContext('2d'), {
      type: 'line',
      data: {
        labels: days.map(d=>d.label),
        datasets: [{
          data: days.map(d=>d.load),
          borderColor: '#f4a261',
          backgroundColor: 'rgba(244,162,97,.08)',
          pointBackgroundColor: days.map(d=>d.load!==null?'#f4a261':'transparent'),
          pointBorderColor: days.map(d=>d.load!==null?'#f4a261':'transparent'),
          pointRadius: days.map(d=>d.load!==null?6:0),
          pointHoverRadius: 8,
          borderWidth: 2,
          tension: 0,
          fill: true,
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor:'#1a1a1a', borderColor:'rgba(255,255,255,.1)', borderWidth:1,
            titleColor:'rgba(240,236,228,.5)', bodyColor:'#f4a261',
            callbacks: {
              label: ctx => ctx.raw !== null ? (hasDuration ? 'Charge : '+Math.round(ctx.raw)+' sRPE' : 'RPE : '+ctx.raw.toFixed(1)) : 'Aucune séance'
            }
          }
        },
        scales: {
  ...
  function renderChargeAcwr(){
    const area = document.getElementById('charge-chart-area');
    if(!area) return;

    const d3ago = new Date(); d3ago.setDate(d3ago.getDate()-2); d3ago.setHours(0,0,0,0);
    const d3Key = dateToKey(d3ago);

    const last3 = _chargeCompletions.filter(c=>c.schedule_date>=d3Key);
    const last21 = _chargeCompletions;

    const sRPE = c => (c.duration_minutes && c.duration_minutes > 0) ? c.rpe_ressenti * c.duration_minutes : c.rpe_ressenti;
    const hasDuration = _chargeCompletions.some(c => c.duration_minutes && c.duration_minutes > 0);

    const acuteLoad  = last3.reduce((s,c)=>s+sRPE(c),0) / 3;
    const chronicLoad = last21.reduce((s,c)=>s+sRPE(c),0) / 21;
    const acwr = chronicLoad > 0 ? acuteLoad / chronicLoad : null;

    let zoneColor='#60a5fa', zoneLabel='—', zoneDesc='Données insuffisantes';
    if(acwr !== null){
      if(acwr < 0.8)       { zoneColor='#60a5fa'; zoneLabel='Sous-charge';    zoneDesc='La charge récente est faible — risque de déconditionnement'; }
      else if(acwr <= 1.3) { zoneColor='#34d399'; zoneLabel='Zone optimale';  zoneDesc='Charge bien équilibrée — progression sans surmenage'; }
      else if(acwr <= 1.5) { zoneColor='#f4a261'; zoneLabel='Attention';      zoneDesc='Pic de charge — surveiller la récupération'; }
      else                 { zoneColor='#e63946'; zoneLabel='Surcharge';      zoneDesc='Risque élevé de blessure — réduire l\'intensité'; }
    }

    const acuteColor  = last3.length  ? RPE_COLORS[Math.min(10,Math.round(hasDuration?acuteLoad/60:acuteLoad))]  : 'rgba(240,236,228,.2)';
    const chronicColor = last21.length ? RPE_COLORS[Math.min(10,Math.round(hasDuration?chronicLoad/60:chronicLoad))] : 'rgba(240,236,228,.2)';
    const acuteDisplay  = last3.length  ? (hasDuration ? Math.round(acuteLoad)  : acuteLoad.toFixed(1))  : '—';
    const chronicDisplay = last21.length ? (hasDuration ? Math.round(chronicLoad) : chronicLoad.toFixed(1)) : '—';
    const acuteUnit  = hasDuration ? 'sRPE/jour' : 'RPE moy.';
    const chronicUnit = hasDuration ? 'sRPE/jour' : 'RPE moy.';
    const pct = acwr !== null ? Math.min(100, Math.max(0, (acwr/2)*100)) : null;

    area.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:16px;text-align:center">
          <div style="font-size:10px;font-weight:700;letter-spacing:.13em;text-transform:uppercase;color:rgba(240,236,228,.3);margin-bottom:8px">Charge aiguë (3j)</div>
          <div style="font-family:'Bebas Neue',sans-serif;font-size:40px;color:${acuteColor};line-height:1">${acuteDisplay}</div>
          <div style="font-size:10px;color:rgba(240,236,228,.25);margin-top:4px">${acuteUnit}</div>
          <div style="font-size:11px;color:rgba(240,236,228,.3);margin-top:3px">${last3.length} séance${last3.length!==1?'s':''}</div>
        </div>
        <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:16px;text-align:center">
          <div style="font-size:10px;font-weight:700;letter-spacing:.13em;text-transform:uppercase;color:rgba(240,236,228,.3);margin-bottom:8px">Charge chronique (21j)</div>
          <div style="font-family:'Bebas Neue',sans-serif;font-size:40px;color:${chronicColor};line-height:1">${chronicDisplay}</div>
          <div style="font-size:10px;color:rgba(240,236,228,.25);margin-top:4px">${chronicUnit}</div>
          <div style="font-size:11px;color:rgba(240,236,228,.3);margin-top:3px">${last21.length} séance${last21.length!==1?'s':''}</div>
        </div>
      </div>
      ${acwr !== null ? `
      <div style="background:rgba(255,255,255,.04);border:1px solid ${zoneColor}55;border-radius:14px;padding:20px;text-align:center;margin-bottom:16px">
        <div style="font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(240,236,228,.3);margin-bottom:8px">Ratio aiguë / chronique</div>
        <div style="font-family:'Bebas Neue',sans-serif;font-size:56px;color:${zoneColor};line-height:1;margin-bottom:6px">${acwr.toFixed(2)}</div>
        <div style="font-size:14px;font-weight:700;color:${zoneColor};margin-bottom:4px">${zoneLabel}</div>
        <div style="font-size:12px;color:rgba(240,236,228,.4)">${zoneDesc}</div>
      </div>
      <div style="position:relative;height:16px;background:rgba(255,255,255,.06);border-radius:8px;overflow:visible;margin-bottom:8px">
        <div style="position:absolute;inset:0;background:linear-gradient(to right,#60a5fa 0%,#34d399 20%,#34d399 43%,#f4a261 57%,#e63946 72%,#e63946 100%);opacity:.35;border-radius:8px"></div>
        <div style="position:absolute;top:50%;left:${pct}%;transform:translate(-50%,-50%);width:18px;height:18px;background:${zoneColor};border-radius:50%;border:3px solid #080808;box-shadow:0 0 8px ${zoneColor}88;transition:left .4s"></div>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:9px;color:rgba(240,236,228,.25);letter-spacing:.06em;margin-top:4px">
        <span>0</span><span>0.8</span><span>1.3</span><span>1.5</span><span>2+</span>
      </div>` : `
      <div style="padding:30px;text-align:center;color:rgba(240,236,228,.25);font-size:13px">
        Données insuffisantes — l'athlète doit compléter des séances avec RPE.
      </div>`}`;
  }

  /* ══ GRAPHIQUE PROFIL — SÉANCES 30 JOURS ══ */
  let _profilChart = null;

  async function renderProfilChart(){
    const el = document.getElementById('profil-chart-section');
    if(!el || !currentUser) return;
    if(_profilChart){ _profilChart.destroy(); _profilChart = null; }

    const d30ago = new Date(); d30ago.setDate(d30ago.getDate()-29); d30ago.setHours(0,0,0,0);
    const d30Key = dateToKey(d30ago);

    if(currentUser.role === 'coach') await renderCoachProfilChart(el, d30Key);
    else if(currentUser.role === 'athlete') await renderAthleteProfilChart(el, d30Key);
    else el.innerHTML = '';
  }

  async function renderCoachProfilChart(el, d30Key){
    const data = window._lastCoachData;
    if(!data || !data.athletes.length){
      el.innerHTML = `<div class="wide-card" style="margin-top:24px">
        <h3 style="margin:0 0 8px;font-size:14px;font-weight:600;color:rgba(240,236,228,.6)">Séances — 30 derniers jours</h3>
        <p style="color:rgba(240,236,228,.3);font-size:13px;margin:0">Aucun athlète enregistré.</p>
      </div>`;
      return;
    }

    const barH = Math.max(160, data.athletes.length * 38);
    el.innerHTML = `
      <div class="wide-card" style="margin-top:24px;padding:20px 20px 16px">
        <h3 style="margin:0 0 4px;font-size:14px;font-weight:700;letter-spacing:.04em;color:var(--white)">Séances — 30 derniers jours</h3>
        <p style="font-size:12px;color:rgba(240,236,228,.35);margin:0 0 18px">Nombre de séances complétées par athlète</p>
        <div style="position:relative;height:${barH}px">
          <canvas id="profil-chart-canvas"></canvas>
        </div>
      </div>`;

    const athleteIds = data.athletes.map(a=>a.id);
    let completions = [];
    try {
      completions = await sbFetch('seance_completions?athlete_id=in.('+athleteIds.join(',')+')'+'&schedule_date=gte.'+d30Key+'&select=athlete_id') || [];
    } catch(e){}

    const combined = data.athletes.map(a => ({
      label: a.firstName + ' ' + (a.lastName?.[0]||'') + '.',
      count: completions.filter(c => c.athlete_id === a.id).length
    })).sort((a,b) => b.count - a.count);

    const canvas = document.getElementById('profil-chart-canvas');
    if(!canvas) return;
    _profilChart = new Chart(canvas.getContext('2d'), {
      type: 'line',
      data: {
        labels: combined.map(x=>x.label),
        datasets: [{
          data: combined.map(x=>x.count),
          borderColor: '#f4a261',
          backgroundColor: 'rgba(244,162,97,.08)',
      ...
      options: {
        ...
        scales: {
          x: {
            ticks: { color:'rgba(240,236,228,.35)', font:{family:'Outfit',size:11} },
            grid: { display:false }
          },
          y: {
            beginAtZero: true,
            ticks: { color:'rgba(240,236,228,.35)', font:{family:'Outfit',size:11}, stepSize:1, precision:0 },
  ...
  async function renderAthleteProfilChart(el, d30Key){
    const savedLink = (()=>{ try{ return JSON.parse(localStorage.getItem('athlete_link_'+currentUser.username)||'null'); }catch(e){return null;} })();
    if(!savedLink){
      el.innerHTML = `<div class="wide-card" style="margin-top:24px">
        <h3 style="margin:0 0 8px;font-size:14px;font-weight:600;color:rgba(240,236,228,.6)">Séances — 30 derniers jours</h3>
        <p style="color:rgba(240,236,228,.3);font-size:13px;margin:0">Synchronisez votre compte avec un coach pour voir vos statistiques.</p>
      </div>`;
      return;
    }
    ...
    el.innerHTML = `
      <div class="wide-card" style="margin-top:24px;padding:20px 20px 16px">
        <h3 style="margin:0 0 4px;font-size:14px;font-weight:700;letter-spacing:.04em;color:var(--white)">Mes séances — 30 derniers jours</h3>
        <p style="font-size:12px;color:rgba(240,236,228,.35);margin:0 0 18px">Séances complétées par semaine</p>
    ...
    let completions = [];
    try {
      completions = await sbFetch('seance_completions?athlete_id=eq.'+savedLink.athleteId+'&schedule_date=gte.'+d30Key+'&select=schedule_date') || [];
    } catch(e){}
    ...
    for(let i=4; i>=0; i--){
      ...
      const sun = new Date(mon); sun.setDate(mon.getDate()+6);
      const monKey = dateToKey(mon), sunKey = dateToKey(sun);
      const count = completions.filter(r => r.schedule_date >= monKey && r.schedule_date <= sunKey).length;
      weeks.push({ label: mon.getDate()+'/'+(mon.getMonth()+1), count });
    }
  ...
  /* ══ GRAPHIQUE PROFIL ATHLÈTE (vue coach) ══ */
  let _adpAthleteChart = null;

  async function renderAdpAthleteChart(athleteId){
    const el = document.getElementById('adp-profil-chart-section');
    if(!el) return;
    if(_adpAthleteChart){ _adpAthleteChart.destroy(); _adpAthleteChart = null; }
    ...
    el.innerHTML = `
      <div class="wide-card" style="margin-top:4px;padding:20px 20px 16px">
        <h3 style="margin:0 0 4px;font-size:14px;font-weight:700;letter-spacing:.04em;color:var(--white)">Séances — 30 derniers jours</h3>
        <p style="font-size:12px;color:rgba(240,236,228,.35);margin:0 0 18px">Séances complétées par semaine</p>
        <div style="position:relative;height:180px">
          <canvas id="adp-profil-chart-canvas"></canvas>
        </div>
      </div>`;

    let completions = [];
    try {
      completions = await sbFetch('seance_completions?athlete_id=eq.'+athleteId+'&schedule_date=gte.'+d30Key+'&select=schedule_date') || [];
    } catch(e){ return; }
    ...
    const canvas = document.getElementById('adp-profil-chart-canvas');
    if(!canvas) return;
    _adpAthleteChart = new Chart(canvas.getContext('2d'), {
      type: 'line',
      data: {
        labels: weeks.map(w=>w.label),
        datasets: [{
  ...
  function openRpeBubble(seanceId, dateKey){
    ...
    const durInput = document.getElementById('rpe-duration');
    if(durInput) durInput.value = '';
    document.getElementById('rpe-bubble-overlay').classList.add('open');
  }

  function rpeCheckConfirm(){
    const dur = parseInt(document.getElementById('rpe-duration')?.value);
    const btn = document.getElementById('rpe-confirm');
    if(btn) btn.disabled = (_rpeSelected === null || !dur || dur < 1);
  }
  ...
  async function rpeConfirm(){
    ...
    const duration = parseInt(document.getElementById('rpe-duration')?.value) || null;
    try {
      const key = _rpeSeanceId+'|'+_rpeDateKey;
      const comp = (athData.completions||{})[key];
      if(comp){
        try {
          const patch = {rpe_ressenti: _rpeSelected};
          if(duration) patch.duration_minutes = duration;
          await sbFetch('seance_completions?id=eq.'+comp.id, 'PATCH', patch);
        } catch(e2){
          // Fallback si duration_minutes n'existe pas encore
          await sbFetch('seance_completions?id=eq.'+comp.id, 'PATCH', {rpe_ressenti: _rpeSelected});
        }
        athData.completions[key].rpe = _rpeSelected;
        if(duration) athData.completions[key].duration = duration;
      }
  ...
  /* ── SEANCE PICKER (ajout rapide au planning) ── */
  function openSeancePicker(e, dateKey){
    e.stopPropagation();
    closeSeancePicker();

    const popup = document.createElement('div');
    popup.id = 'seance-picker-popup';
    ...
    const d = keyToDate(dateKey);
    const DAYS_FR = ['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'];
    const dateLabel = DAYS_FR[d.getDay()]+' '+d.getDate()+'/'+(d.getMonth()+1);

    popup.innerHTML = `
      <div class="sp-header">Ajouter au planning — ${dateLabel}</div>
      <div id="sp-list" style="padding:4px 0;color:rgba(240,236,228,.3);font-size:12px;padding:10px">⏳ Chargement…</div>`;
    document.body.appendChild(popup);

    getAthleteData().then(({athlete}) => {
      const sp = document.getElementById('sp-list');
      if(!sp) return;
      const seances = athlete?.seances || [];
      if(!seances.length){
        sp.innerHTML = `<div style="padding:10px;font-size:12px;color:rgba(240,236,228,.3)">Aucune séance disponible.<br>Créez d'abord une séance.</div>`;
        return;
      }
      sp.innerHTML = seances.map(s => `
        <button class="sp-item" onclick="pickSeanceForDay('${s.id}','${dateKey}')">
          <div class="sp-bar" style="background:${s.color||'#2ec4b6'}"></div>
          <div class="sp-info">
            <div class="sp-name">${escHtml(s.title)}</div>
            <div class="sp-type">${s.type==='ligne'?'Ligne':'Block'}</div>
          </div>
        </button>`).join('');
    });

    setTimeout(() => document.addEventListener('click', closeSeancePicker, { once: true }), 10);
  }

  function closeSeancePicker(){
    document.getElementById('seance-picker-popup')?.remove();
  }

  async function pickSeanceForDay(seanceId, dateKey){
    closeSeancePicker();
    weekSeanceId = seanceId;
    await addSeanceToDay(dateKey);
  }
